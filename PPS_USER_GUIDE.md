# Инструкция для ППС: Codex app + Git marketplace + Sylabys Syllabus Checker
...
Плагин устанавливается через Git marketplace внутри Codex app. Команды `codex plugin ...` для ППС не используются. Git Bash открывать не нужно. Но сам Git for Windows должен быть установлен, иначе Codex app не сможет добавить marketplace из GitHub.
...
3. В правом верхнем углу откройте выпадающее меню со стрелкой `>` / `⌄`. Нажимать нужно именно стрелку меню рядом с кнопкой, а не просто плюс.
...
В новом чате вставьте такой текст. Замените путь и название файла на свои.
...
3. Установить Git for Windows.
4. Открыть `Плагины`.
5. В правом верхнем углу открыть выпадающее меню со стрелкой `>` / `⌄`, а не просто нажать плюс.
